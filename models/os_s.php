...
switch($dados['acao']) {
    ...
    case 'edit':
        ...
        if (Database::update('os', $os, ['id'=>$os['id']])) {
            if (!Database::remove('osxservicos', ['idos'=>$os['id']])) {
    ...
    case 'remove':
        ...
        if (!Database::remove('osxservicos', ['idos'=>$dados_['id']])) {
            Query::error('Não foi possivel deletar as entradas na relação, OS x Servicos', true);
        }
        if (!Database::remove('os', ['id'=>$dados_['id']])) {
    ...
    default:
        $sth = $db->prepare("select 
						...
						from os 
							...
							LEFT join garantias on garantias.id = os.garantia WHERE os.status=0	");
        $sth->execute();
