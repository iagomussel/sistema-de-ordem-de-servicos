...
if(!isset($dados["acao"])) $dados["acao"] = "view";
			switch($dados["acao"]){
				...
				case "get_all":
					...
					$stmt = $db->prepare("SELECT * FROM condpagtos where id=:id");
				...
				case "remove":
						...
						$stmt = $db->prepare("DELETE FROM condpagtos WHERE id = :id");
						$stmt->bindValue(":id",$dados_["id"]);
						if($stmt->execute()){
							Query::data($dados_);
							Query::json();
				...
				break;
				default:
					$sth = $db->prepare("select condpagtos.*,formpagtos.descricao as formpagto_descricao from condpagtos INNER JOIN formpagtos on formpagtos.id = condpagtos.formpagto");
					$sth->execute();
