...
carregamento = 9;
tiposutil = false;
system_url = "http://localhost/vendas/";
...
mf.relatorios.forms = {
	...
	faturamento:{data_inicial:"",data_final:""}
	}
		...
		callback:function(a){
			...
			if(a.error){
				...
				if(a.request.acao=='add'){
					...
					mf.os_s.push(dat);
					mf.forms.os_s = {nome:"",cidade:4204,estado:19,telefone:"",cnpj:"",num:0,bairro:"",ie:"",status:"",im:"",email:"",endereco:"",cep:""};
					...
					dat.funcionario = mf.funcionarios[mf.funcionarios.indexOfOb("id",""+dat.funcionario)].nome
					dat.garantia = mf.garantias[mf.garantias.indexOfOb("id",""+dat.garantia)].descricao
					vue.mf.os_s.$set(mf.os_s.indexOfOb("id",""+dat.id),dat)
					mf.forms.os_s = {nome:"",cidade:4204,estado:19,telefone:"",cnpj:"",num:0,bairro:"",ie:"",status:"",im:"",email:"",endereco:"",cep:""};
				} 
		...
		callback:function(a){
			...
			if(a.error){
				...
				if(a.request.acao=='add'){
					...
					mf.forms.orcamentos = {nome:"",cidade:4204,estado:19,telefone:"",cnpj:"",num:0,bairro:"",ie:"",status:"",im:"",email:"",endereco:"",cep:""};
				} else if(a.request.acao=='edit'){
					dat = a.data;
					dat.cliente = mf.clientes[mf.clientes.indexOfOb("id",""+dat.cliente)].nome
					dat.veiculo = mf.veiculos[mf.veiculos.indexOfOb("id",""+dat.veiculo)].placa
		...
		callback:function(a){
			...
			if(a.error){
				...
				if(a.request.acao=='add'){
					...
					dat.cidade = mf.cidade[mf.cidade.indexOfOb("Id",""+dat.cidade)].Nome
					dat.estado = mf.estados[mf.estados.indexOfOb("Id",""+dat.estado)].Uf
					vue.mf.clientes.$set(mf.clientes.indexOfOb("id",""+dat.id),dat)
				} 
		...
		callback:function(a){
			...
			if(a.error){
				...
				if(a.request.acao=='add'){
					...
					var dat = a.data;
					dat.clienteNome = mf.clientes[mf.clientes.indexOfOb("id",""+dat.cliente)].nome
					vue.mf.veiculos.$set(mf.veiculos.indexOfOb("id",dat.id),dat)
				} 
		...
		callback:function(a){
			...
			if(a.error){
				...
				if(a.request.acao=='add'){
					...
					var dat = a.data;
					vue.mf.funcionarios.$set(mf.funcionarios.indexOfOb("id",""+dat.id),dat)
				} 
		...
		callback:function(a){
			...
			if(a.error){
				...
				if(a.request.acao=='add'){
					...
					var dat = a.data;
					vue.mf.formpagtos.$set(mf.formpagtos.indexOfOb("id",""+dat.id),dat)
				} 
		...
		callback:function(a){
			...
			if(a.error){
				...
				if(a.request.acao=='add'){
					...
					var dat = a.data;
					dat.formpagto_descricao = mf.formpagtos[mf.formpagtos.indexOfOb("id",dat.formpagto)].descricao;
					vue.mf.condpagtos.$set(mf.condpagtos.indexOfOb("id",dat.id),dat)
				} 
		...
		callback:function(a){
			...
			if(a.error){
				...
				if(a.request.acao=='add'){
					...
					var dat = a.data;
					vue.mf.garantias.$set(mf.garantias.indexOfOb("id",dat.id),dat)
				} 
...
remove_funcionario = function(ind){
	if(ind==undefined)return false;
	save({
		local:"funcionarios",
		dados:{id:mf.funcionarios[ind].id},
...
mf.relatorios.faturamento = function(){
	...
	mf.relatorios.forms.faturamento.ordenar_por = "";
	mf.relatorios.forms.faturamento.graficos = "";
	mf.relatorios.forms.faturamento.tipo_graf = "";
	$("#form_faturamento_modal").modal("show").find("input,textarea").val("");
}


mf.relatorios.visualizar = function(local,ind){
